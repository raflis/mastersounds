@extends('admin.master')

@section('content')

<div class="layoutContent">
    <div class="container-fluid">
        <div class="row layout-header">
            <div class="col-sm-12 header-content">
                <h1>
                    <i class="fas fa-bullhorn fa-xs text-white2"></i> Usuarios
                </h1>
                <span class="subtitle">
                    Permisos
                </span>
            </div>
        </div>
        <div class="row layout-body">
            <div class="col-sm-12">
                <div class="card shadow">
                    <div class="card-header">
                        <span>
                            Editar permisos
                        </span>
                    </div>
                    {!! Form::model($user, ['route' => ['users.permission.update', $user->id], 'method' => 'PUT', 'class' => 'needs-validation', 'novalidate', 'files' => true]) !!}
                    <div class="card-body row justify-content-start">
                        <div class="col-md-12">
                            <h6>Usuario: {{ $user->name }} {{ $user->lastname }}</h6>
                            <h6>Perfil: {{ getRole($user->role) }}</h6>
                        </div>
                        <div class="col-sm-12">
                            @include('admin.includes.alert')
                        </div>
                        @include('admin.users.partials.roles')
                    </div>
                </div>
            </div>
            <div class="col-sm-12 my-4">
                <span class="d-block mb-3 font-semibold"><code>*</code> Campos Obligatorios</span>
                {!! Form::submit('Guardar cambios',['class'=>'btn btn-success py-2 px-3']) !!}
                <a class="btn btn-danger py-2 px-3" href="{{ route('users.index') }}">Atrás</a>
                {!! Form::close() !!}
            </div>
        </div>
        @include('admin.includes.footer')
    </div>
</div>

@endsection