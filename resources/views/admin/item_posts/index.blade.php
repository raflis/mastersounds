    ...
    <div class="container-fluid">
        <div class="row layout-header">
            <div class="col-sm-12 header-content">
                <h1>
                    <i class="fas fa-bullhorn fa-xs text-white2"></i> Noticias
                </h1>
                <span class="subtitle">
            ...
            <div class="col-sm-12">
                <div class="card shadow">
                    <div class="card-header">
                        ...
                        <div>
                            @if (validatePermission('item_posts.create', Auth::user()->permissions) == true)
                            <a class="btn btn-success" href="{{ route('item_posts.create') }}">
                                <span class="icon">
                ...
                <div class="tablaTotal">
                    <table class="table tableD">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $item_posts->firstItem() + $loop->index }}</td>
                                <td>
                                    {{ $item->category_post->name1 }}<br/>
                                    <i>{{ $item->category_post->name2 }}</i>
                                </td>
                                <td>
                                    {{ $item->name1 }}<br/>
                                    <i>{{ $item->name2 }}</i>
                                ...
                                <td>
                                    <div style="display: inline-flex">
                                        @if (validatePermission('item_posts.edit', Auth::user()->permissions) == true)
                                        <a class="btn btn-primary text-white btn-sm mr-1" href="{{ route('item_posts.edit', $item->id) }}">
                                            <i class="far fa-edit pr-1"></i> Editar
                                        </a>
                                        @endif
                                        @if (validatePermission('item_posts.delete', Auth::user()->permissions) == true)
                ...
                <div class="paginacionTotal d-flex justify-content-end">
                    {{ $item_posts->withQueryString()->render() }}
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer justify-content-center">
            ...
            {!! Form::open(['route' => ['item_posts.destroy', ''], 'method' => 'DELETE', 'id' => 'form-eliminar']) !!}
...
$('document').ready(function(){
    $('.btn-eliminar').click(function(e){
        ...
        var base = '{{ route('item_posts.destroy', '') }}';
        var url = base + '/' +id;
