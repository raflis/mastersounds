            ...
            <div class="col-sm-12">
                <div class="card shadow">
                    <div class="card-header">
                        ...
                        <div>
                            @if (validatePermission('item_solutions.create', Auth::user()->permissions) == true)
                            <a class="btn btn-success" href="{{ route('item_solutions.create') }}">
                                <span class="icon">
                ...
                <div class="tablaTotal">
                    <table class="table tableD">
                        ...
                        <tbody>
                            @foreach ($item_solutions as $item)
                            <tr>
                                <td>{{ $item_solutions->firstItem() + $loop->index }}</td>
                                <td>
                                    {{ $item->category_solution->name1 }}<br/>
                                    <i>{{ $item->category_solution->name2 }}</i>
                                </td>
                                <td>
                                    {{ $item->name }}
                                </td>
                                <td>
                                    {!! nl2br(htmlspecialchars_decode($item->description1)) !!}<br/>
                                    <i>{!! nl2br(htmlspecialchars_decode($item->description2)) !!}</i><br/>
                                    <b>Países:</b>
...
{{$country->name}}
                                ...
                                <td>
                                    <div style="display: inline-flex">
                                        @if (validatePermission('item_solutions.edit', Auth::user()->permissions) == true)
                                        <a class="btn btn-primary text-white btn-sm mr-1" href="{{ route('item_solutions.edit', $item->id) }}">
                                            <i class="far fa-edit pr-1"></i> Editar
                                        </a>
                                        @endif
                                        @if (validatePermission('item_solutions.delete', Auth::user()->permissions) == true)
                ...
                <div class="paginacionTotal d-flex justify-content-end">
                    {{ $item_solutions->withQueryString()->render() }}
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer justify-content-center">
            ...
            {!! Form::open(['route' => ['item_solutions.destroy', ''], 'method' => 'DELETE', 'id' => 'form-eliminar']) !!}
...
$('document').ready(function(){
    $('.btn-eliminar').click(function(e){
        ...
        var base = '{{ route('item_solutions.destroy', '') }}';
        var url = base + '/' +id;
